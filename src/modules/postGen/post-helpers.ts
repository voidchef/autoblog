...
import {
  extractJsonArray,
  extractCodeBlock,
  extractPostOutlineFromCodeBlock,
  extractSeoInfo,
  extractAudienceIntentInfo,
} from './extractor';
import {
...
export interface GeneratorHelperInterface {
  init(): Promise<void>;
  isCustom(): boolean;
  setSystemMessage(): Promise<void>;
  setCompletionParams(): Promise<void>;
...
export class ChatGptHelper implements GeneratorHelperInterface {
  ...
  async setSystemMessage() {
    this.chatParentMessage = await this.sendRequest(this.systemMessage!);
  }

  async setCompletionParams() {
    const completionParams = getPromptForChatParameters(this.completionParams);
    this.chatParentMessage = await this.sendRequest(completionParams);
  }
  ...
  async generateContentOutline() {
    ...
    // By this way, we can minimize the cost of the API call by minimizing the number of prompt tokens
  ...
  async generateIntroduction() {
    const response = await this.sendRequest(getPromptForIntroduction(this.postPrompt));
    return extractCodeBlock(response.text);
  }

  async generateConclusion() {
    const response = await this.sendRequest(getPromptForConclusion());
    return extractCodeBlock(response.text);
  }
  ...
  async generateHeadingContents(postOutline: PostOutline) {
    ...
    return this.buildContent(postOutline.headings, headingLevel);
  }
  ...
  private async getContent(heading: Heading): Promise<string> {
    ...
    const response = await this.sendRequest(getPromptForHeading(this.postPrompt.tone!, heading.title, heading.keywords!));
    return `${extractCodeBlock(response.text)}\n`;
  }
  ...
  async generateCustomPrompt(customPrompt: string) {
    this.chatParentMessage = await this.sendRequest(customPrompt);
  ...
  async generateSeoInfo(): Promise<SeoInfo> {
    ...
    this.chatParentMessage = await this.sendRequest(getPromptForSeoInfo());
    if (this.postPrompt.debug) {
      log('---------- SEO INFO ----------');
      console.log(this.chatParentMessage.text);
    }
    ...
    return pRetry(
      async () => {
        const options: SendMessageOptions = {
          parentMessageId: this.chatParentMessage?.id,
          conversationId: this.chatParentMessage?.conversationId as string,
        };
  ...
  private manageError(error: FailedAttemptError) {
    ...
    if (error instanceof ChatGPTError) {
      ...
      if (chatGPTError.statusCode === 401) {
        console.log(
          'OpenAI API Error : Invalid API key: please check your API key in the option -k or in the OPENAI_API_KEY env var.',
        );
        process.exit(1);
      }
    ...
    if (error instanceof AbortError) {
      ...
      console.log(
        `OpenAI API - Request failed - Attempt ${error.attemptNumber} failed. There are ${error.retriesLeft} retries left. ${error.message}`,
      );
    }
  }
}
