...
import { PostPrompt, CompletionParams } from './types';

const STRUCTURE_OUTLINE =
...
const INFORMATIVE_INTRO_PROMPT =
  'Compose the introduction for this blog post topic, without using phrases such as, "In this article,..." to introduce the subject.' +
...
export function getPromptForIntentAudience(postPrompt: PostPrompt) {
  return (
    `For a post based on the topic : ${postPrompt.topic}, describe the ideal audience and intent for this topic.` +
...
export function getPromptForChatParameters(completionParams: CompletionParams) {
  return (
    `Use "temperature" value of ${completionParams.temperature} in our conversation.\n` +
    `Use "frequency_penalty" value of ${completionParams.frequency_penalty} in our conversation.\n` +
    `Use "frequencyPenalty" value of ${completionParams.presence_penalty} in our conversation.`
  );
}
...
export function getPromptForOutline(postPrompt: PostPrompt) {
  ...
  const prompt =
    `${STRUCTURE_OUTLINE}For the title and headings, do not capitalize words unless the first one.` +
    ...
    }${audience ? `Audience: ${audience}.` : ''}${intent ? `Content intent: ${intent}.` : ''}`;
...
export function getPromptForMainKeyword() {
  const prompt =
    'Give me the most important SEO keyword in a JSON array in which each item matches a word without the stop words.';
  return prompt;
}
...
export function getPromptForConclusion() {
  return (
    'Write a compelling conclusion for this blog post topic without using transitional phrases such as, "In conclusion", "In summary", "In short", "So", "Thus", or any other transitional expressions.' +
...
function getPromptForInformativeHeading(title: string, keywords: string[] | null) {
  ...
  return (
    `Write some informative content for the heading (without the heading)  "${title}"${promptAboutKeywords}Do not start the first sentence with the heading. Instead, start with a sentence that introduces and provides context for the heading.` +
...
function getPromptForCaptivatingHeading(title: string, keywords: string[] | null) {
  ...
  return (
    `Write some captivating content for the heading (without the heading): "${title}"${promptAboutKeywords}Make sure to provide in-depth information and valuable insights. Use clear and concise language, along with relevant examples or anecdotes, to engage the reader and enhance their understanding.` +
...
export function getPromptForHeading(tone: string, title: string, keywords: string[] | null) {
  return tone === 'informative'
    ? getPromptForInformativeHeading(title, keywords)
    : getPromptForCaptivatingHeading(title, keywords);
}
...
export function getSeoSystemPrompt(postPrompt: PostPrompt) {
  return (
    `You are a SEO expert and you need to optimize a web page based on the following info in ${postPrompt.language}: ` +
    `\n${postPrompt.prompts![0]}`
  );
}

export function getPromptForSeoInfo() {
  return (
    'For content based on the topic of this conversation, Use the H1 provided in the system prompt. If not, write a new H1. ' +
